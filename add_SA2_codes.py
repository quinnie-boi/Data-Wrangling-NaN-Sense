...
def main():
    ...
    if test_result is None:
        print("Warning: test query did not return an SA2 code.")
        print("Check the API key, layer ID, and coordinate values.")
        sys.exit(1)

    print("Processing all rows...")

    df = add_sa2_codes(df, api_key)
...
def test_first_500_rows(df, api_key):
    ...
    test_df = add_sa2_codes(test_df, api_key)

    missing_count = test_df[OUTPUT_COLUMN].isna().sum()

    print("\nTest completed.")
    print(f"Rows processed: {len(test_df)}")
    print(f"Rows matched: {len(test_df) - missing_count}")
